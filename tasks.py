...
def coverage(c):
    ...
    """
    c.run("coverage run --source dj-jb-zerodowntime runtests.py tests")
...
def docs(c):
    ...
    """
    c.run("rm -f docs/dj-jb-zerodowntime.rst")
    c.run("rm -f docs/modules.rst")
    c.run("sphinx-apidoc -o docs/ dj_jb_zerodowntime")
...
def lint(c):
    ...
    """
    c.run("flake8 dj-jb-zerodowntime tests")
...
def release(c, bumpsize=''):
    ...
    import dj_jb_zerodowntime
    c.run("python setup.py sdist bdist_wheel")
    c.run("twine upload dist/*")

    c.run('git tag -a {version} -m "New version: {version}"'.format(version=dj_jb_zerodowntime.__version__))
    c.run("git push --tags")
