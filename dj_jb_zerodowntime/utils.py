...
UnsafeMigration = namedtuple(
    "UnsafeMigration", ("app_name", "migration_name", "offending_operations", "sql_statements")
)

MigrationConflict = namedtuple(
    "MigrationConflict",
    (
        "app_name",
        "migration_names",
...
def analyze_migration(connection, migration, project_state):
    ...
    if len(unsafe_operations) > 0:
        ...
        return UnsafeMigration(
            migration.app_label, migration.name, unsafe_operations, sql_statements
        )
...
def find_unsafe_migrations(connection):
    ...
    unsafe_migrations = sorted(
        unsafe_migrations, key=operator.attrgetter("app_name", "migration_name")
    )
