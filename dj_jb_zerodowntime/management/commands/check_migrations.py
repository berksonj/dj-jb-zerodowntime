...
from ...utils import MigrationConflict, UnsafeMigration, find_unsafe_migrations
...
def print_migration_conflict(migration):
    print(
        "\n\nMigration Conflict found in app: {app}, migrations: {migrations}:".format(
            app=migration.app_name, migrations=migration.migration_name
        )
    )
...
class Command(BaseCommand):
    help = (
        "Analyzes pending migrations to determine whether or not they are compatible with a"
